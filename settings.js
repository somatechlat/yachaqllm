const settingsModalProxy = {
    ...
    async openModal() {
        ...
        try {
            ...
            window.toastFetchError("Error al obtener configuración", e)
        }
    },
    ...
    async handleButton(buttonId) {
        if (buttonId === 'save') {
            ...
            try {
                ...
                window.toastFetchError("Error al guardar configuración", e)
                return
            }
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data && data.settings) {
                            ...
                            console.error('Formato de datos de configuración inválido');
                        }
                    } else {
                        console.error('Error al obtener configuración:', response.statusText);
                    }
                } catch (error) {
                    ...
                    for (const section of this.settingsData.sections) {
                        for (const field of section.fields) {
                            if (field.required && (!field.value || field.value.trim() === '')) {
                                showToast(`${field.title} en ${section.title} es requerido`, 'error');
                                return;
                            }
                        }
                    }
